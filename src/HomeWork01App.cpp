...
/**
*Retrived sudo code from Wikipedia - http://en.wikipedia.org/wiki/Bresenham%27s_line_algorithm
*Satisfies A.3
**/
void HomeWork01App::drawLine(uint8_t* pixels, int x0, int x1, int y0, int y1, int width)
{
	if(x0 < 0 || y0 < 0 || y1 < 0 || x1 < 0 || x0 > kAppHeight || x1 > kAppHeight || y1 > kAppHeight || 
		y0>kAppHeight ||
		x0>kAppWidth ||
		x1>kAppWidth ||
		y0>kAppWidth ||
